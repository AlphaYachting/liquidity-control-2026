...
export function buildLLMPrompt(llmContext) {
  ...
  const lines = [
    ...
    `  Noch offen netto (gesamt): ${fmtEur(com.total_order_net - fin.already_invoiced_net)}`,
    `  Sicher verfügbar netto (nach Abzug aktiver Anweisungen): ${overlap ? fmtEur(overlap.safe_remaining_to_invoice_net) : fmtEur(com.total_order_net - fin.already_invoiced_net)}`,
    `  Aktive Anweisungen (noch nicht verrechnet): ${overlap ? fmtEur(overlap.active_instruction_amount_net) : '€ 0,00'}`,
