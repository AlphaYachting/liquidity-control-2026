...
export default function ProjectDetail() {
  ...
  // awork data object (project fields take priority over order fallback)
  const aworkData = {
    awork_project_id: effectiveAworkProjectId,
    awork_project_name: project.awork_project_name || primaryOrder?.awork_project_name,
    awork_project_status: project.awork_project_status || primaryOrder?.awork_project_status,
    awork_progress_percent: project.awork_progress_percent || primaryOrder?.awork_progress_percent || 0,
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <KpiCard title="Verrechnet (netto)" value={formatCurrency(adjustedInvoicedNet)} variant="success"
          subtitle={`${realInvoices.length} Rechnung(en)`} />
        <KpiCard title="Abrechnungsbereit" value={formatCurrency(readyAmount)} variant={readyAmount > 0 ? 'warning' : 'default'}
          subtitle={readyBlocks.length > 0 ? `${readyBlocks.length} Paket(e)` : undefined} />
                  ...
                  <table className="w-full text-sm">
                    ...
                    <tfoot>
                      <tr className="border-t bg-muted/20">
                        ...
                        <td className="py-2 text-right font-bold text-amber-600">{formatCurrency(Math.max(0, openReceivable))}</td>
                        <td colSpan={2}></td>
                      </tr>
                    </tfoot>
                  </table>
